...
%     or [BX, BY, BZ] = IGRF1(COEFS, LATITUDE, LONGITUDE, ALTITUDE, COORD)
...
% If only one output is requested, B = [BX(:) BY(:) BZ(:)] is output. Note
% that the other parameters the IGRF gives can be computed from BX, BY, and
% BZ as:
% 
%   Horizonal intenisty: sqrt(BX.^2 + BY.^2)
%   Total intensity: sqrt(BX.^2 + BY.^2 + BZ.^2)
%   Declination: atan(BY ./ BX)
%   Inclination: atan(BZ ./ sqrt(BX.^2 + BY.^2))
% 
...
if nargin < 5 || isempty(coord) || strcmpi(coord, 'geodetic') || ...
    ...
    % a = sqrt(40680631.59); b = sqrt(40408299.98);
