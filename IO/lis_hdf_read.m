function [ flashes, viewtime, groups ] = lis_hdf_read( filename )
...
%   Written By:  Michael Hutchins
	...
	groups = hdfread(filename,'group');
	
	time = double(groups{1}');
	location = double(groups{3}');
	radiance = double(groups{4}');
	
	date = TAI93ToUTC(time);
	
	groups = [datevec(date),location,radiance];
